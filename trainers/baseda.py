...
class BaseDA(TrainerXU):
    ...
    def test(self, split=None):
        ...
        #     if "image_encoder.transformer.resblocks.0.attn.q_proj.w_lora_A" in name:  # 如需更精确可写 "w_lora"
        #         print(param)
        # 在循环外部初始化总时间和总处理图片数
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            #batch_size = input.size(0)  # 获取当前批次的图片数量
            ...
            # 打印当前批次的推理时间
           # print(f"Batch {batch_idx} inference time: {batch_inference_time:.4f} seconds, "f"batch size: {batch_size}")
            self.evaluator.process(output, label)
        '''if total_images_processed > 0:
            avg_time_per_image = total_inference_time / total_images_processed
            print(f"\n=== 推理统计 ===")
            print(f"总处理图片数: {total_images_processed}")
            print(f"总推理时间: {total_inference_time:.4f} 秒")
            print(f"平均单张推理时间: {avg_time_per_image:.4f} 秒")
            print(f"平均每秒处理图片数 (FPS): {1.0 / avg_time_per_image:.2f}")
        exit()'''
