...
class LoRALayer():
    def __init__(
        ...
        # T-LoRA: optional dynamic rank mask, shape [r].
        self._tlora_mask = None

    def set_tlora_mask(self, active_rank: Optional[int] = None, mask: Optional[torch.Tensor] = None):
        """Set timestep-dependent LoRA rank mask.

        - active_rank: enable first active_rank channels.
        - mask: custom mask tensor with shape [r] or [1, r].
        """
        if self.r <= 0:
            return

        if mask is not None:
            rank_mask = mask.detach().float().view(-1)
            if rank_mask.numel() != self.r:
                raise ValueError(f"T-LoRA mask size mismatch: expected {self.r}, got {rank_mask.numel()}")
            self._tlora_mask = rank_mask
            return
        ...
        active_rank = max(1, min(int(active_rank), self.r))
        rank_mask = torch.zeros(self.r, dtype=torch.float32)
        rank_mask[:active_rank] = 1.0
        self._tlora_mask = rank_mask

    def clear_tlora_mask(self):
        self._tlora_mask = None

    def _get_tlora_mask(self, device, dtype):
        if self.r <= 0:
            return None
        if self._tlora_mask is None:
            return torch.ones(self.r, device=device, dtype=dtype)
        return self._tlora_mask.to(device=device, dtype=dtype)
    ...
    def init_lora_param(self):
        """
        用 SoRA-SVD 初始化 lora_A / lora_B，并把残差写回原权重。
        完全替代父类默认的 kaiming_uniform + zeros。
        """
        if self.r <= 0:
            return
        ...
        for param_name, lora_name in self.params_with_lora.items():
            W: nn.Parameter = eval(f'self.{param_name}')
            dtype = W.dtype
            device = W.device
            # 1. 升到 float32 做 SVD
            weight_f32 = W.data.to(torch.float32)

            U, S, Vh = torch.linalg.svd(weight_f32, full_matrices=False)

            # 2. 挑奇异值
            if self.first_eigen:  # 主成分
                Ur, Sr, Vhr = U[:, :self.r], S[:self.r], Vh[:self.r, :]
            else:  # 次要成分（SoRA 默认）
                Ur, Sr, Vhr = U[:, -self.r:], S[-self.r:], Vh[-self.r:, :]

            Sr = Sr / self.scaling  # 先缩掉 scaling，前向再乘回来

            # 3. 造 lora_A / lora_B
            sqrt_S = torch.diag(torch.sqrt(Sr))
            lora_A = sqrt_S @ Vhr  # (r, d_in)
            lora_B = Ur @ sqrt_S  # (d_out, r)

            # 4. 写回可训练参数
            getattr(self, f'{lora_name}_lora_A').data = lora_A.to(dtype)
            getattr(self, f'{lora_name}_lora_B').data = lora_B.to(dtype)

            # 5. 残差写回原权重并冻结
            residual = weight_f32 - self.scaling * (lora_B @ lora_A)
            W.data = residual.to(dtype)
    ...
    def merge_BA(self, param_name: str):
        ...
        lora_B = eval(f'self.{lora_name}_lora_B')
        lora_A = eval(f'self.{lora_name}_lora_A')
        rank_mask = self._get_tlora_mask(lora_B.device, lora_B.dtype)
        if rank_mask is not None:
            lora_B = lora_B * rank_mask.view(1, -1)
        return self.transpose((lora_B @ lora_A).view(eval(f'self.{param_name}').shape))
...
class Conv2d(nn.Conv2d, LoRALayer):
    """Conv2d LoRA wrapper that works for both legacy and new constructors."""

    def __init__(
        self,
        existing_conv: Optional[nn.Conv2d] = None,
        *,
        ...
        **kwargs,
        ...
        if existing_conv is not None:
            ...
            if in_channels is None or out_channels is None or kernel_size is None:
                raise ValueError("Conv2d LoRA requires either an existing_conv or explicit dimensions")
            nn.Conv2d.__init__(
                self,
        ...
        # 【关键修复】: 如有必要，覆盖 LoRALayer 的默认设置
        # 我们的假设是：对于 Conv2d，应该 *总是* 调整最不重要的成分，
        # 以保护那些基础的预训练滤波器。
        self.params_with_lora = {'weight': 'w'}
        if r > 0:
            self.register_lora_param_conv2d()
            self.init_lora_param()
    ...
    def register_lora_param_conv2d(self):
        weight_shape = self.weight.shape
        in_dim = self.weight.view(weight_shape[0], -1).shape[1]
        out_dim = weight_shape[0]
        self.register_parameter('w_lora_A', nn.Parameter(self.weight.new_zeros((self.r, in_dim))))
        self.register_parameter('w_lora_B', nn.Parameter(self.weight.new_zeros((out_dim, self.r))))
        self.weight.requires_grad = False

    def merge_BA(self, param_name: str):
        lora_name = self.params_with_lora[param_name]
        lora_A = getattr(self, f'{lora_name}_lora_A')
        lora_B = getattr(self, f'{lora_name}_lora_B')
        rank_mask = self._get_tlora_mask(lora_B.device, lora_B.dtype)
        if rank_mask is not None:
            lora_B = lora_B * rank_mask.view(1, -1)
        delta_w_2d = lora_B @ lora_A
        return delta_w_2d.view(self.weight.shape)
    # 新增一个简单的标准初始化方法
    def init_lora_param_simple(self):
        if self.r > 0:
            # 标准 LoRA 初始化
            import math
    ...
    def init_lora_param(self):
        ...
        W: nn.Parameter = self.weight
        dtype = W.dtype
        device = W.device
        weight_matrix = W.data.view(W.shape[0], -1).to(torch.float32)
        ...
        sqrt_S = torch.diag(torch.sqrt(Sr))
        lora_A = sqrt_S @ Vhr  # (r, d_in_flattened)
        lora_B = Ur @ sqrt_S  # (d_out, r)

        self.w_lora_A.data = lora_A.to(dtype)
        self.w_lora_B.data = lora_B.to(dtype)

        residual_matrix = weight_matrix - self.scaling * (lora_B @ lora_A)
        W.data = residual_matrix.view(W.shape).to(dtype)
        W.requires_grad = False
        reconstructed_matrix = residual_matrix + self.scaling * (lora_B @ lora_A)
        difference = torch.mean(torch.abs(weight_matrix - reconstructed_matrix))
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        original_output = nn.Conv2d.forward(self, x, **kwargs)
        if self.training and self.dropout.p > 0:
            x = self.dropout(x)

        if self.r > 0 and not self.merged:
            delta_w = self.merge_BA('weight')
            lora_adjustment = F.conv2d(
                x,
                delta_w,
                bias=None,
                stride=self.stride,
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        self.q_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).half()
        self.k_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).half()
        self.v_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.in_proj_bias is not None).half()
        self.proj = nn.Linear(self.embed_dim, self.embed_dim, bias=existing_mha.out_proj.bias is not None).half()
...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            ...
            return nn.Linear.forward(self, x, **kwargs)
class BatchNormLoRA(nn.BatchNorm2d):

    def __init__(
        self,
        existing_bn: nn.BatchNorm2d,
        r: int = 0, 
        lora_alpha: int = 1, 
    ):
        super().__init__(
            num_features=existing_bn.num_features,
            eps=existing_bn.eps,
            momentum=existing_bn.momentum,
            affine=existing_bn.affine, 
            track_running_stats=existing_bn.track_running_stats
        )
        
        self.load_state_dict(existing_bn.state_dict())
        self.to(device=existing_bn.weight.device, dtype=existing_bn.weight.dtype)
