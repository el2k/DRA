...
def load_clip_to_cpu(cfg, teacher=False):
    if teacher:
...
class Simple_TextEncoder(nn.Module):
    ...
    def forward(self, text, prompts=None):
        if prompts != None:
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, deep_prompts=None):
        ...
        # text_embedding_at_eos = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection  # [n_cls, 512]
        # text_embedding_all = torch.einsum('kld,dc->klc', x, self.text_projection)  # [n_cls, 77, 512]
        ...
        if self.deep:
            if deep_x.dim == 2:
                if self.deep_layer == None:
                    ...
                    deep_x = deep_x.unsueeze(0).expand(self.deep_layer[1] - self.deep_layer[0] + 1, -1,
                                                       -1)  # only specified layers exsit prompt
